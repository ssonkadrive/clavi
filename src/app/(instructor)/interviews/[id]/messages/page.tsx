...
export default function MessagesPage() {
  ...
  console.log('[MessagesPage] 마운트됨, proposalId:', proposalId)
  ...
  useEffect(() => {
    if (!proposalId) {
      console.log('[MessagesPage] proposalId가 없습니다.')
      setIsLoading(false)
      return
    }

    const loadMessages = async () => {
      console.log('[MessagesPage] 메시지 로드 시작:', proposalId)
      setIsLoading(true)
      setError('')

      try {
        const result = await getMessages(proposalId)
        console.log('[MessagesPage] 메시지 로드 결과:', {
        ...
        console.error('[MessagesPage] 메시지 로드 실패:', err)
